...
async def mori_ingest_content(
    ...
    """Run content-based ingestion — extract durable memories from
    base64-encoded file bytes sent over the wire.

    Alternative to mori-ingest for remote clients where the server
    cannot access the client device filesystem. Accepts file bytes
    ...
    Args:
        files: List of file objects with keys: name, content_b64, mime_type.
        focus: What to extract — all, decisions, architecture, conventions, gotchas.
        tier: Memory tier — working (default), canonical, ephemeral.
        tags: Comma-separated tags to apply to produced memories.
        dry_run: If true, calls the LLM but does not write anything.
    ...
    if not files:
        return "No files provided. Pass a list of {name, content_b64, mime_type} objects."

    try:
        result = ingestion_pipeline.ingest_content(
            files=files,
            focus=focus,
            tier=tier,
            tags=parsed_tags,
        ...
        parts = [
            f"**Content ingestion {'preview (dry-run)' if dry_run else 'complete'}**",
            f"  Files processed:    {result['sources']}",
            f"  Chunks sent:        {result['chunks']}",
            f"  Skipped (cached):   {result['skipped']}",
            f"  Errors:             {result['errors']}",
            f"  Est. cost:          ${result['cost_estimate']:.4f}",
        ...
        if error_details:
            ...
            if len(error_details) > 5:
                parts.append(f"  ... and {len(error_details) - 5} more")

        return "\n".join(parts)

    except Exception as e:
        logger.error("mori-ingest-content failed: %s", e)
        return f"Ingestion failed: {e}"
