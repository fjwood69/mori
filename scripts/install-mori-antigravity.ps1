    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($RawContent)) {
            ...
            if ($null -eq $Existing.mcpServers) {
                ...
                # Handle NoteProperty conversion safely
            ...
            $Existing | ConvertTo-Json -Depth 10 | Set-Content -Path $McpConfigPath -Encoding UTF8
...
$HooksConfig = [PSCustomObject]@{
    hooks = [PSCustomObject]@{
        PostToolUse = @(
            [PSCustomObject]@{
                type = "command"
                command = "curl -sf -X POST `"$MoriUrl/api/events/raw?client=$ClientName`" $($AuthHeader)-H `"Content-Type: application/json`" -d @- >nul 2>&1; exit 0"
            }
        )
        PostToolUseFailure = @(
            [PSCustomObject]@{
        ...
        PreCompact = @(
            [PSCustomObject]@{
                type = "command"
                command = "curl -sf -X POST `"$MoriUrl/api/precompact?client=$ClientName`" $($AuthHeader)-H `"Content-Type: application/json`" -d @- >nul 2>&1; exit 0"
            }
        )
    }
}

# Remove existing hooks.json if it was corrupted to force a clean write
if (Test-Path $HooksPath) {
    Remove-Item $HooksPath -Force
}

$HooksConfig | ConvertTo-Json -Depth 10 | Set-Content -Path $HooksPath -Encoding UTF8
